...
import {UiNode, UiContainer, UiNodeInputAttributes} from '@ory/kratos-client';

import {FormBuilderSubmitPayload, makeLink} from '@client/shared/common';
import {isInputAttributes} from '../helpers';
    ...
    template: `
        @if (formUI) {
            <form [formGroup]="form">
                @if (formUI.messages) {
                    <div class="mb-4">
                        <fr-form-messages [messages]="formUI.messages"/>
                    </div>
                }
                        ...
                        @if (node.type === 'input' && node.attributes.node_type === "input") {
                            @if (node.attributes.type !== 'hidden' && node.attributes.type !== 'submit') {
                                <div class="mb-4">
                                    ...
                                    />
                                    @if (node.messages) {
                                        <fr-form-messages [messages]="node.messages"/>
                                    }
                                </div>
                            }
