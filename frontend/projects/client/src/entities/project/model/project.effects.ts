...
import {
    ProjectCreateAction,
    ProjectListGetAction, ProjectPatchAction, ProjectUpdateAction,
    ProjectSetAction,
    ...
    (
        ...
        return actions$.pipe(
            ...
            exhaustMap(() => service.List().pipe(
                switchMap(data => of(ProjectListSetAction({payload: data}))),
            ))
        );
    },
    ...
    (
        ...
        return actions$.pipe(
            ...
            exhaustMap((action) => service.Create(action.payload).pipe(
                switchMap(result => of(ProjectSetAction({payload: result}))),
            ))
        )
    },
    {functional: true, dispatch: true}
...
export const ProjectPatchEffect = createEffect(
    (
        ...
        return actions$.pipe(
            ...
            exhaustMap(action => service.Patch(action.projectId, action.payload).pipe(
                switchMap(data => of(ProjectUpdateAction({payload: data}))),
            ))
        );
    },
    {functional: true, dispatch: true}
