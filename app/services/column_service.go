...
func (s *ColumnService) Filter(ctx context.Context, boardId uuid.UUID) []models.Column {
	...
	if result := persistence.GetDB(ctx).Preload("Tasks").Where("board_id = ?", boardId).Find(&columns); result.Error != nil {
...
func (s *ColumnService) Get(ctx context.Context, boardListID uuid.UUID) (*models.Column, error) {
	...
	if result := persistence.GetDB(ctx).Where("id = ?", boardListID).First(&column); result.Error != nil {
...
func (s *ColumnService) Reorder(ctx context.Context, listID uuid.UUID, order uint) error {
	...
	return persistence.GetDB(ctx).Model(&models.Column{}).
		Where("id = ?", listID).
...
func (s *ColumnService) Create(ctx context.Context, payload ColumnUpdate) (*models.Column, error) {
	...
	if result := persistence.GetDB(ctx).Create(&column); result.Error != nil {
		return nil, result.Error
	}
	return &column, nil
}
...
func (s *ColumnService) Update(ctx context.Context, column *models.Column) (*models.Column, error) {
	if result := persistence.GetDB(ctx).Save(&column); result.Error != nil {
		return nil, result.Error
	}
	return column, nil
}
...
func (s *ColumnService) Delete(ctx context.Context, id uuid.UUID) error {
	if result := persistence.GetDB(ctx).Where("id = ?", id).Delete(&models.Column{}); result.Error != nil {
		return result.Error
	}
	return nil
}
