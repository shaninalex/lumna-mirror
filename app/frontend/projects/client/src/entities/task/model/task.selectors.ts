...
export const selectTasksByProjectID = (projectID: number) =>
	createSelector(selectAllTasks, tasks => tasks.filter(t => t.project_id === projectID))
export const selectTask = (taskCode: string) => createSelector(selectAllTasks, tasks => tasks.find(t => t.code === taskCode))
