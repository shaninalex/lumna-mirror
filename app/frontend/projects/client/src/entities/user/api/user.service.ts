...
export class UserService {
	...
	getUser(): Observable<UserModel> {
		return this.http
			.get<APIResponse<UserModel>>(`${environment.API_ROOT}/api/v1/user/me`, { withCredentials: true })
			.pipe(map(data => data.data))
	}

	updateUserSettings(settings: Settings): Observable<UserModel> {
		return this.http
			.post<APIResponse<UserModel>>(`${environment.API_ROOT}/api/v1/user/settings`, settings, { withCredentials: true })
			.pipe(map(data => data.data))
	}
