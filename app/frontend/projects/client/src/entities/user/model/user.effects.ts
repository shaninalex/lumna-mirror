...
import { UserGetAction, UserSetAction, UserUpdateSettingsAction } from "@client/entities/user/model/user.actions"
...
export const UserUpdateSettingsEffect = createEffect(
	(actions$ = inject(Actions), service = inject(UserService)) => {
		return actions$.pipe(
			...
			exhaustMap(action =>
				service.updateUserSettings(action.payload).pipe(
