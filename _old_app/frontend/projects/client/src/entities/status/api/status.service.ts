...
export class StatusService extends CommonApiService {
    public List(projectId: number): Observable<Status[]> {
        return this.get<Status[]>(statusUrl.statusRoot(projectId)).pipe(map(data => data.data))
    }

    public Create(projectId: number, payload: StatusInput): Observable<Status> {
        return this.post<Status>(statusUrl.statusRoot(projectId), payload).pipe(map(data => data.data))
    }

    public Patch(projectId: number, statusId: number, payload: StatusInput): Observable<Status> {
        return this.patch<Status>(statusUrl.statusItem(projectId, statusId), payload).pipe(map(data => data.data))
    }

    public Delete(projectId: number, statusId: number): Observable<null> {
        return this.delete<null>(statusUrl.statusItem(projectId, statusId)).pipe(map(data => data.data))
    }

    public StatusSort(projectId: number, payload: Record<number, number>): Observable<Status[]> {
        return this.patch<Status[]>(statusUrl.statusSort(projectId), payload).pipe(map(data => data.data))
    }
}
